...
class TwitchTV {
  ...
  var $client_id = 't6a5c7t3yr8usx1yh3kuse4w3uwlq5r'; //change this value, should be your TwitchTV Application Client ID
  var $client_secret = "8pxiklorovyv4y39jcvz03yvefp2vej"; //change this value, should be your TwitchTV Application Client Secret 
  var $redirect_url = 'http://localhost/'; //change this value, should be your TwitchTV Application Rerdirect URL
  var $scope_array = array('user_read', 'user_follows_edit');
...
//echo '<script> alert("'.$ch.'"); </script>';
	 ...
	 * @return boolean
	 */
	public function follow_channel($user, $channel,$access_token) {
		$ch = curl_init($this->base_url . "users/" . $user . "/follows/channels/".$channel);
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
		curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, FALSE);		
		curl_setopt($ch, CURLOPT_HTTPHEADER, array('Accept: application/vnd.twitchtv.v3+json', 'Authorization: OAuth '.$access_token));
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
	...
	/**
	 * Make a request to unfollow a given channel
	 * 
	 * @param string $channel
	 * @param string $access_token
	 * @return boolean
	 */
	public function unfollow_channel($user, $channel, $access_token) {
	...
	/**
	 * Make a request to know if a given user is following a given channel
	 * 
	 * @param string $channel
	 * @param string $access_token
	 * @return boolean
	 */
	public function is_following_channel($channel,$access_token) {
		$username = $this->authenticated_user($access_token);
